...
public class SeminarOntology implements IOntology {
	...
	public String addClass(String synset, String lemmaURI, boolean lex, String lexName, HashSet<String> aspectProperties, String... classURIs ) {
		...
		{
			newClass.addProperty(ontology.getProperty(NS + "#lex"), lexName.toLowerCase());
			String[] lemmas = getLemmasFromSynset(synset);
			
			for (int i = 0; i < lemmas.length; i++)
			{
				String temp = lemmas[i];
				newClass.addProperty(ontology.getProperty(NS + "#lex"), temp.toLowerCase());
			}
		...
		//Add synset property
		newClass.addProperty(ontology.getProperty(NS + "#synset"), synset.replace("@", "#"));
	 ...
	 * @return The URI of the new class
	 */
	public String addClassSynset(String synset, boolean lex, HashSet<String> aspectProperties, String... classURIs ) 
	{	
		String[] lemmas = getLemmasFromSynset(synset);
		
		String lemmaURI = lemmas[0];
		lemmaURI = lemmaURI.substring(0, 1).toUpperCase() + lemmaURI.substring(1);
	...
	public String addClass(String pos, HashMap<String, String> context, String synset, boolean lex, String lexName, HashSet<String> aspectProperties, String... classURIs) { //pos = "noun" or "verb"
		...
		if (lex) {
			...
			{
			newClass.addProperty(ontology.getProperty(NS + "#lex"), lemmas[x].toLowerCase());
			}
		}
		...
		Synset syns = WordSenseDisambiguation.findSynset(contextWords, lexName, pos);
		
		if (syns != null)
		{
		String[] words = syns.getWordForms();
