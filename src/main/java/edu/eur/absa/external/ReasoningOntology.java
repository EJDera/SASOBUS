...
public class ReasoningOntology implements IOntology {
	...
	public HashSet<String> getSenseConcepts(String superclassURI, String lemma){
		return getLexicalizedConcepts(superclassURI, NS+"#sense", lemma);
	}
	...
	public HashMap<String, String> lexToURI(){
		...
		while (iter.hasNext()) {
			...
			Statement stmt = iter.nextStatement();
			Resource  subject   = stmt.getSubject();     // get the subject
			RDFNode sense = stmt.getObject();
			StmtIterator iter2 = ontology.listStatements(new SimpleSelector(
					subject, 
					ontology.getProperty("http://www.w3.org/2000/01/rdf-schema#subClassOf"),
					ontology.getResource(this.URI_Mention)));
			if (iter2.hasNext()){
				ontoConcepts.put(sense.toString(), subject.getURI());
			} else {
	...
	public HashMap<String, String> senseToURI(){
		StmtIterator iter = ontology.listStatements(new SimpleSelector(null, ontology.getProperty(NS+"#sense"),(Literal)null));
